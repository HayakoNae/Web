<template>
	<div class="mod-config">
		...
		<el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle"
			...
			<el-table-column prop="pid" header-align="center" align="center" label="pid">
			</el-table-column>
			<el-table-column prop="title" header-align="center" align="center" label="标题">
			</el-table-column>
			<el-table-column prop="text" header-align="center" align="center" label="文章内容">
			</el-table-column>
...
<script>
	...
	export default {
		...
		activated() {
			this.getDataList();
			this.checkToken();	// 校验token，或者token是否过期
		},
		methods: {
			...
			getDataList() {
				...
				this.$http({
					url: this.$http.adornUrl('https://blog-server-api.amarea.cn/post/getList'),
					method: 'get',
			...
			deleteHandle(id) {
				...
				this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
					...
					this.$http({
						url: this.$http.adornUrl('https://blog-server-api.amarea.cn/post/deletePost'),
						method: 'post',
						data: this.$http.adornData({
							'token': this.$cookie.get('token'),
							'pid': id
